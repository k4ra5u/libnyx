...
use fuzz_runner::nyx::aux_buffer::{
    NYX_ABORT, NYX_CRASH, NYX_INPUT_WRITE, NYX_SUCCESS, NYX_TIMEOUT,
};
...
impl NyxConfig {
    ...
    pub fn print(&self) {
        ...
        println!(
            ...
            self.input_buffer_write_protection()
        );
...
impl NyxProcess {
    ...
    pub fn quic_response_buffer(&self) -> &[u8] {
        self.process.quic_response
    }

    pub fn quic_response_buffer_mut(&mut self) -> &mut [u8] {
        self.process.quic_response
    }

    pub fn execution_path_buffer(&self) -> &[u8] {
        self.process.execution_path
    }

    pub fn execution_path_buffer_mut(&mut self) -> &mut [u8] {
        self.process.execution_path
    }
    ...
    pub fn aux_string(&self) -> String {
        ...
        String::from_utf8_lossy(&self.process.aux_buffer().misc_data_slice()[0..len as usize])
            .to_string()
    }
    ...
    pub fn exec(&mut self) -> NyxReturnValue {
        match self.process.send_payload() {
            ...
            Ok(_) => match self.process.aux_buffer().result.exec_result_code {
    ...
    pub fn set_input_ptr(&mut self, buffer: *const u8, size: u32) {
        unsafe {
            std::ptr::copy(
                &size,
                self.process.payload.as_mut_ptr() as *mut u32,
                1 as usize,
            );
            std::ptr::copy(
                buffer,
                self.process.payload[std::mem::size_of::<u32>()..].as_mut_ptr(),
                std::cmp::min(size as usize, self.input_buffer_size()),
            );
        }
    }
